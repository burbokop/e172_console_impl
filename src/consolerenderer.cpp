...
void ConsoleRenderer::drawPixel(const e172::Vector<double> &point, e172::Color color) {
    pixel_primitives::pixel(m_writer.bitmap(), point.x(), point.y()) = color;
}

void ConsoleRenderer::drawLine(const e172::Vector<double> &point0,
...
void ConsoleRenderer::drawRect(const e172::Vector<double> &point0,
                               const e172::Vector<double> &point1,
                               uint32_t color,
...
void ConsoleRenderer::drawSquare(const e172::Vector<double> &point, int radius, uint32_t color)
{
    pixel_primitives::draw_square(m_writer.bitmap(), point.x(), point.y(), radius, color);
}

void ConsoleRenderer::drawCircle(const e172::Vector<double> &point, int radius, uint32_t color) {
    pixel_primitives::draw_circle(m_writer.bitmap(), point.x(), point.y(), radius, color);
}

void ConsoleRenderer::drawImage(const e172::Image &image,
...
void ConsoleRenderer::modifyBitmap(const std::function<void(e172::Color *)> &modifier)
{
...
void ConsoleRenderer::setResolution(e172::Vector<double> value) {
    m_writer.set_frame_size(value.x(), value.y());
}

e172::Vector<double> ConsoleRenderer::resolution() const
{
    return e172::Vector<double>(m_writer.bitmap().width, m_writer.bitmap().height);
}

e172::Vector<double> ConsoleRenderer::screenSize() const {
